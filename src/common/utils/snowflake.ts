...
export class SnowflakeIdGenerate {
    ...
    private maxWorkerId = -1 ^ (-1 << this.workerIdBits); // 值为：1
    ...
    constructor(_workerId = 0, _dataCenterId = 0, _sequence = 0) {
        if (this.workerId > this.maxWorkerId || this.workerId < 0) {
            throw new Error('config.worker_id must max than 0 and small than maxWorkerId-[' + this.maxWorkerId + ']');
        }
    ...
    private nextId = (): number => {
        ...
        const timestampPos = (timestamp - this.epoch) * 4096;
